...
use mfsimvr_gui_common::theme;
use mfsimvr_packets::{ClientConnectionsAction, PathValuePair};
...
impl Dashboard {
    pub fn new(creation_context: &eframe::CreationContext<'_>, data_sources: DataSources) -> Self {
        mfsimvr_gui_common::theme::set_theme(&creation_context.egui_ctx);
                        ...
                        SetupWizardRequest::Close { finished } => {
                            if finished {
                                requests.push(ServerRequest::SetSessionValues(vec![
                                    PathValuePair {
                                        path: mfsimvr_packets::parse_path(
            ...
            Panel::left("side_panel")
                ...
                .show_inside(ui, |ui| {
                    ui.with_layout(Layout::top_down_justified(Align::Center), |ui| {
                        ui.add_space(13.0);
                        ui.heading(RichText::new("MFSimVR").size(25.0).strong());
