use mfsimvr_common::{LogEntry, LogSeverity};
use mfsimvr_gui_common::theme::{self, log_colors};
use mfsimvr_session::Settings;
use eframe::{
...
const NOTIFICATION_TIPS: &[&str] = &[
    ...
    r#"If you started having crashes after changing some settings, reset MFSimVR by re-running "Run setup wizard" from the "Installation" tab and clicking "Reset settings"."#,
    r#"Some settings are hidden by default. Click the "Expand" button next to some settings to expand the submenus."#,
    r#"It's highly advisable to keep audio settings as default in MFSimVR and modify the default audio device in the taskbar tray."#,
    ...
    r#"If you have problems syncing external controllers or trackers to MFSimVR tracking space, add one element to "Headset"->"Extra OpenVR properties", then set a custom "Tracking system name string"."#,
    r#"To change the visual appearance of controllers, set "Headset"->"Controllers"->"Emulation mode"."#,
    r#"MFSimVR supports custom button bindings! If you need help, please ask us on our Discord server."#,
    r#"MFSimVR supports hand tracking gestures ("Presets"->"Hand tracking interaction"->"MFSimVR bindings"). Check out wiki how to use them properly: https://github.com/alvr-org/ALVR/wiki/Hand-tracking-controller-bindings."#,
