...
use eframe::egui;
use mfsimvr_common::{
    MFSIMVR_VERSION, RelaxedAtomic, debug, error, info,
    parking_lot::Mutex,
    semver::{Version, VersionReq},
    warn,
};
use mfsimvr_events::{Event, EventType};
use mfsimvr_packets::FirewallRulesAction;
use mfsimvr_server_io::ServerSessionManager;
use serde::Serialize;
use std::{
    io::ErrorKind,
...
pub fn clean_session() {
    ...
    if session_manager.session().server_version != *MFSIMVR_VERSION {
        let mut session_ref = session_manager.session_mut();
        session_ref.server_version = MFSIMVR_VERSION.clone();
        session_ref.client_connections.clear();
        session_ref.session_settings.extra.open_setup_wizard = true;

        // CORREÇÃO: Removido o `.content` e ajustado para o novo padrão de nomenclatura (se houver)
        // Se o campo mudou de nome junto com o projeto, ele provavelmente se chama `hide_while_version` direto no `.extra`
        session_ref.session_settings.extra.hide_while_version = MFSIMVR_VERSION.to_string();
    }
}
        ...
        let version_check_thread = thread::spawn({
            ...
            move || {
                let version_requirement = {
                    ...
                    // CORREÇÃO AQUI: Pegamos diretamente o campo de texto da versão de dentro de .extra
                    let version = &session_manager_lock.settings().extra.hide_while_version;
                        ...
                        if let SessionSource::Local(session_manager) = &mut *session_source.lock() {
                            match request {
                                ...
                                ServerRequest::AddFirewallRules => {
                                    if let Err(e) = mfsimvr_server_io::firewall_rules(
                                ...
                                ServerRequest::RegisterAlvrDriver => {
                                    ...
                                    mfsimvr_server_io::driver_registration(
                                        &[alvr_driver_dir],
                                        true,
                                    )
                                    .ok();

                                    if let Ok(list) = mfsimvr_server_io::get_registered_drivers() {
                                        report_event_local(
                                            &context,
                                            &events_sender,
                                ...
                                ServerRequest::UnregisterDriver(path) => {
                                    mfsimvr_server_io::driver_registration(&[path], false).ok();
        ...
        let events_thread = thread::spawn({
            ...
            move || {
                while running.value() {
                    ...
                    req.headers_mut()
                        .insert("X-MFSimVR", HeaderValue::from_str("true").unwrap());
            ...
            move || {
                ...
                loop {
                    ...
                    let connected = if let Some(version) = maybe_server_version {
                        ...
                        let matches = version == *mfsimvr_common::MFSIMVR_VERSION;

                        if !matches {
                            error!(
                                "Server version mismatch: found {version}. Please remove all previous MFSimVR installations"
                            );
                        }
