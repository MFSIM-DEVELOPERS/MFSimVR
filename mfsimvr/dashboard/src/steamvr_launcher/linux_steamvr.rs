...
use mfsimvr_common::anyhow::bail;
use mfsimvr_common::{debug, error, info, warn};
use sysinfo::Process;
...
pub fn maybe_wrap_vrcompositor_launcher() -> mfsimvr_common::anyhow::Result<()> {
    let steamvr_bin_dir = mfsimvr_server_io::steamvr_root_dir()?
        .join("bin")
        .join("linux64");
    ...
    match steamvr_vrserver_path.try_exists() {
        Ok(exists) => {
            if !exists {
                bail!(
                    "SteamVR Linux files missing, aborting startup, please re-check compatibility tools for SteamVR, verify integrity of files for SteamVR and make sure you're not using Flatpak Steam with non-Flatpak MFSimVR."
                );
            }
        }
        Err(e) => {
...
fn linux_gpu_checks(device_infos: &[(&wgpu::Adapter, DeviceInfo)]) {
    ...
    let steamvr_root_dir = match mfsimvr_server_io::steamvr_root_dir() {
        Ok(dir) => dir,
        Err(e) => {
            error!(
                "Couldn't find OpenVR or SteamVR files. \
                Please make sure you have installed and ran SteamVR at least once. \
                Or if you're using Flatpak Steam, make sure to use mfsimvr Dashboard from Flatpak MFSimVR. {e}"
            );
            return;
        }
    };
        ...
        match device_info.1 {
            DeviceInfo::Nvidia => {
                match nvml_wrapper::Nvml::init() {
                    ...
                    Err(e) => {
                        mfsimvr_common::show_e(format!("Can't initialize NVML engine, error: {e}."))
                    }
                }
            }
