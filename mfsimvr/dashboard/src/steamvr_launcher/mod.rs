...
use mfsimvr_adb::commands as adb;
use mfsimvr_common::{
...
use mfsimvr_filesystem::{self as afs};
...
fn unblock_alvr_driver() -> Result<()> {
    ...
    let path = mfsimvr_server_io::steamvr_settings_file_path()?;
...
impl Launcher {
    pub fn launch_steamvr(&self) {
        ...
        let other_alvr_dirs = mfsimvr_server_io::get_registered_drivers()
            .unwrap_or_default()
            .into_iter()
            .filter(|path| {
                path.to_string_lossy().to_lowercase().contains("MFSimVR") && *path != alvr_driver_dir
            })
            .collect::<Vec<_>>();
        mfsimvr_server_io::driver_registration(&other_alvr_dirs, false).ok();

        mfsimvr_server_io::driver_registration(&[alvr_driver_dir], true).ok();

        if let Err(err) = unblock_alvr_driver() {
            warn!("Failed to unblock MFSimVR driver: {:?}", err);
        }

        #[cfg(target_os = "linux")]
        {
            let vrcompositor_wrap_result = linux_steamvr::maybe_wrap_vrcompositor_launcher();
            mfsimvr_common::show_err(linux_steamvr::maybe_wrap_vrcompositor_launcher());
            ...
            .direct_launch
        {
            ...
            if start_script.exists() {
                ...
                mfsimvr_server_io::steamvr_root_dir().map(|root| root.join("bin"))
            {
