...
pub static MFSIMVR_VERSION: LazyLock<Version> =
...
pub fn is_nightly() -> bool {
    MFSIMVR_VERSION.build.contains("nightly")
}

pub fn is_stable() -> bool {
    MFSIMVR_VERSION.pre.is_empty() && !is_nightly()
}
...
pub fn protocol_id() -> String {
    if MFSIMVR_VERSION.pre.is_empty() {
        MFSIMVR_VERSION.major.to_string()
    } else {
        format!("{}-{}", MFSIMVR_VERSION.major, MFSIMVR_VERSION.pre)
    }
}
