...
use mfsimvr_packets::{ButtonValue, Haptics};
use mfsimvr_server_core::{
...
pub unsafe extern "C" fn HmdDriverFactory(
    ...
    let Ok(driver_dir) = mfsimvr_server_io::get_driver_dir_from_registered() else {
        return ptr::null_mut();
    };
    let Some(filesystem_layout) =
        mfsimvr_filesystem::filesystem_layout_from_openvr_driver_root_dir(&driver_dir)
    else {
        return ptr::null_mut();
    };
