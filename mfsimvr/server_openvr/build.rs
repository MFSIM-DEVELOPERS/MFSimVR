...
fn get_ffmpeg_path(platform_name: &str) -> PathBuf {
    let ffmpeg_path = mfsimvr_filesystem::deps_dir()
        .join(if platform_name == "linux" {
            "linux"
        } else {
            "windows"
...
fn get_linux_x264_path() -> PathBuf {
    mfsimvr_filesystem::deps_dir().join("linux/x264/alvr_build")
}
...
fn main() {
    ...
    build
        ...
        .include(mfsimvr_filesystem::workspace_dir().join("openvr/headers"))
        .include("cpp");
    ...
    if platform_name == "windows" {
        let vpl_path = mfsimvr_filesystem::deps_dir().join("windows/libvpl/alvr_build");
    ...
    if platform_name == "linux" {
        println!(
            "cargo:rustc-link-search=native={}",
            mfsimvr_filesystem::workspace_dir()
    ...
    for path in walkdir::WalkDir::new(mfsimvr_filesystem::workspace_dir().join("openvr/headers"))
        .into_iter()
        .filter_map(|e| e.ok())
        .map(|e| e.into_path())
    {
