...
use mfsimvr_common::{ConResult, HandleTryAgain, ToCon, anyhow::Result, con_bail};
use mfsimvr_session::{DscpTos, SocketBufferConfig};
use bincode::config;
...
pub fn connect_to_client(
    ...
    let mut res = mfsimvr_common::try_again();
    ...
    let recv_cursor_ref = if let Some(cursor) = recv_cursor {
        ...
        loop {
            ...
            if count == FRAMED_PREFIX_LENGTH {
                ...
                return mfsimvr_common::try_again();
            }
        }
