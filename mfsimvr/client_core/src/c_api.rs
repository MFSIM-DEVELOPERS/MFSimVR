...
use mfsimvr_graphics::{
    GraphicsContext, HandData, LobbyRenderer, LobbyViewParams, SDR_FORMAT_GL, StreamRenderer,
    StreamViewParams,
};
use mfsimvr_packets::{ButtonEntry, ButtonValue, FaceData, TrackingData};
use mfsimvr_session::{
...
pub extern "C" fn alvr_path_string_to_id(path: *const c_char) -> u64 {
    mfsimvr_common::hash_string(unsafe { CStr::from_ptr(path) }.to_str().unwrap())
}
...
pub extern "C" fn alvr_dbg_client_impl(message: *const c_char) {
    mfsimvr_common::dbg_client_impl!("{}", unsafe { CStr::from_ptr(message) }.to_str().unwrap())
}

#[unsafe(no_mangle)]
#[cfg_attr(not(debug_assertions), expect(unused_variables))]
pub extern "C" fn alvr_dbg_decoder(message: *const c_char) {
    mfsimvr_common::dbg_decoder!("{}", unsafe { CStr::from_ptr(message) }.to_str().unwrap())
}
...
pub extern "C" fn alvr_mdns_service(service_buffer: *mut c_char) -> u64 {
    string_to_c_str(service_buffer, mfsimvr_sockets::MDNS_SERVICE_TYPE)
}
...
pub extern "C" fn alvr_try_get_permission(permission: *const c_char) {
    mfsimvr_system_info::try_get_permission(unsafe { CStr::from_ptr(permission) }.to_str().unwrap());
}
...
pub extern "C" fn alvr_initialize(capabilities: AlvrClientCapabilities) {
    ...
    let capabilities = ClientCapabilities {
        platform: mfsimvr_system_info::platform(None, None),
...
pub extern "C" fn alvr_send_view_params(view_params: *const AlvrViewParams) {
    if let Some(context) = &*CLIENT_CORE_CONTEXT.lock() {
        context.send_view_params(unsafe {
            [
                mfsimvr_common::from_capi_view_params(&(*view_params)),
                mfsimvr_common::from_capi_view_params(&(*view_params.offset(1))),
            ]
        });
    }
}
...
pub extern "C" fn alvr_send_tracking(
    ...
    let device_motions = raw_motions
        ...
        .map(|motion| {
            (
                motion.device_id,
                DeviceMotion {
                    pose: mfsimvr_common::from_capi_pose(&motion.pose),
            ...
            .map(|&hand_skeleton| {
                (!hand_skeleton.is_null()).then(|| {
                    ...
                    for (pose, capi_pose) in array.iter_mut().zip(hand_skeleton.iter()) {
                        *pose = Pose {
                            orientation: mfsimvr_common::from_capi_quat(&capi_pose.orientation),
    ...
    let eyes_combined = if !combined_eye_gaze.is_null() {
        Some(mfsimvr_common::from_capi_quat(unsafe { &*combined_eye_gaze }))
    } else {
        None
    };
...
pub extern "C" fn alvr_report_compositor_start(
    ...
    if let Some(context) = &*CLIENT_CORE_CONTEXT.lock() {
        ...
        unsafe {
            *out_view_params = mfsimvr_common::to_capi_view_params(&view_params[0]);
            *out_view_params.offset(1) = mfsimvr_common::to_capi_view_params(&view_params[1]);
        }
    }
}
...
pub extern "C" fn alvr_start_stream_opengl(config: AlvrStreamConfig) {
    ...
    STREAM_RENDERER.set(Some(StreamRenderer::new(
        GRAPHICS_CONTEXT.with_borrow(|c| c.as_ref().unwrap().clone()),
        view_resolution,
        mfsimvr_graphics::compute_target_view_resolution(view_resolution, &upscaling),
        swapchain_textures,
        SDR_FORMAT_GL,
        foveated_encoding,
        true,
    ...
    let view_inputs = unsafe {
        [
            LobbyViewParams {
                ...
                view_params: mfsimvr_common::from_capi_view_params(&(*view_inputs).view_params),
            },
            LobbyViewParams {
                swapchain_index: (*view_inputs.offset(1)).swapchain_index,
                view_params: mfsimvr_common::from_capi_view_params(
                ...
                [
                    StreamViewParams {
                        ...
                        input_view_params: ViewParams {
                            pose: Pose::IDENTITY,
                            fov: mfsimvr_common::from_capi_fov(&left_params.fov),
                        },
                        output_view_params: ViewParams {
                            pose: Pose {
                                orientation: mfsimvr_common::from_capi_quat(
                    ...
                    StreamViewParams {
                        ...
                        input_view_params: ViewParams {
                            pose: Pose::IDENTITY,
                            fov: mfsimvr_common::from_capi_fov(&right_params.fov),
                        },
                        output_view_params: ViewParams {
                            pose: Pose {
                                orientation: mfsimvr_common::from_capi_quat(
...
pub extern "C" fn alvr_rotation_delta(source: AlvrQuat, destination: AlvrQuat) -> AlvrQuat {
    mfsimvr_common::to_capi_quat(
        &(mfsimvr_common::from_capi_quat(&source).inverse()
            * mfsimvr_common::from_capi_quat(&destination)),
    )
}
