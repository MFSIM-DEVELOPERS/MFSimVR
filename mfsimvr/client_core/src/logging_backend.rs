...
use mfsimvr_packets::ClientControlPacket;
use std::{
    sync::{LazyLock, mpsc},
...
pub fn init_logging() {
    fn send_log(record: &Record) -> bool {
        ...
        if !mfsimvr_common::filter_debug_groups(&message, &data.debug_groups_config) {
            return false;
        }
    ...
    #[cfg(target_os = "android")]
    {
        android_logger::init_once(
            android_logger::Config::default()
                ...
                .with_max_level(mfsimvr_common::log::LevelFilter::Info),
        );
    }
