...
impl ClientCoreContext {
    pub fn new(capabilities: ClientCapabilities) -> Self {
        ...
        if Config::load().protocol_id != mfsimvr_common::protocol_id() {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            mfsimvr_system_info::try_get_permission(mfsimvr_system_info::MICROPHONE_PERMISSION);
            mfsimvr_system_info::set_wifi_lock(true);
        }
...
impl Drop for ClientCoreContext {
    fn drop(&mut self) {
        ...
        *self.lifecycle_state.write() = LifecycleState::ShuttingDown;
        ...
        #[cfg(target_os = "android")]
        mfsimvr_system_info::set_wifi_lock(false);
    }
}
