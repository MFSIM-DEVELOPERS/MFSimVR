...
use mfsimvr_common::{
    MFSIMVR_VERSION, AnyhowToCon, ConResult, ConnectionError, ConnectionState, LifecycleState,
...
use mfsimvr_session::{SocketProtocol, settings_schema::Switch};
use mfsimvr_sockets::{
...
fn connection_pipeline(
    ...
    let (mut proto_control_socket, server_ip) = {
        ...
        let listener_socket =
            mfsimvr_sockets::get_server_listener(HANDSHAKE_ACTION_TIMEOUT).to_con()?;
    ...
    *connection_state_lock = ConnectionState::Connecting;
    ...
    let microphone_sample_rate =
        mfsimvr_audio::input_sample_rate(&mfsimvr_audio::new_input(None).to_con()?).to_con()?;
    ...
    proto_control_socket
        .send(&ClientConnectionResult::ConnectionAccepted(Box::new(
            ConnectionAcceptedInfo {
                client_protocol_id: mfsimvr_common::protocol_id_u64(),
    ...
    *ctx.statistics_manager.lock() = Some(StatisticsManager::new(
    ...
    let game_audio_thread = if let Switch::Enabled(config) = settings.audio.game_audio {
        let device = mfsimvr_audio::new_output(None).to_con()?;
        thread::spawn({
            let ctx = Arc::clone(&ctx);
            move || {
                while is_streaming(&ctx) {
                    mfsimvr_common::show_err(audio::play_audio_loop(
                        || is_streaming(&ctx),
                        &device,
                        2,
    ...
    let microphone_thread = if matches!(settings.audio.microphone, Switch::Enabled(_)) {
        let device = mfsimvr_audio::new_input(None).to_con()?;
        ...
        move || {
            ...
            while is_streaming(&ctx) && *lifecycle_state.read() == LifecycleState::Resumed {
                ...
                if Instant::now() > battery_deadline {
                    let (gauge_value, is_plugged) = mfsimvr_system_info::get_battery_status();
                    if let Some(sender) = &mut *ctx.control_sender.lock() {
                        sender
