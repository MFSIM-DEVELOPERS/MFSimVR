...
use mfsimvr_graphics::HandData;
use mfsimvr_packets::{ButtonEntry, ButtonValue, ClientStreamConfig, FaceData, FaceExpressions};
use mfsimvr_session::{BodyTrackingBDConfig, BodyTrackingSourcesConfig, FaceTrackingSourcesConfig};
use openxr as xr;
use std::{
...
impl InteractionContext {
    pub fn new(
        ...
        let controllers_profile_id = mfsimvr_common::hash_string(controllers_profile_path);
        ...
        let eyes_combined =
            if extra_extensions::supports_eye_gaze_interaction(&xr_session, xr_system) {
                if matches!(platform, Platform::QuestPro) {
                    ...
                    mfsimvr_system_info::try_get_permission("com.oculus.permission.EYE_TRACKING");
                } else if matches!(
                    platform,
                    Platform::PicoNeo3 | Platform::Pico4Pro | Platform::Pico4Enterprise
                ) {
                    #[cfg(target_os = "android")]
                    mfsimvr_system_info::try_get_permission("com.picovr.permission.EYE_TRACKING");
                }
    ...
    pub fn select_sources(&mut self, config: &InteractionSourcesConfig) {
        ...
        if let Some(config) = &config.face_tracking {
                ...
                && matches!(config, FaceTrackingSourcesConfig::PreferFullFaceTracking)
            {
                #[cfg(target_os = "android")]
                {
                    mfsimvr_system_info::try_get_permission("android.permission.RECORD_AUDIO");
                    mfsimvr_system_info::try_get_permission("com.oculus.permission.FACE_TRACKING")
                }
            }
                ...
                && extra_extensions::supports_eye_gaze_interaction(&self.xr_session, self.xr_system)
            {
                ...
                {
                    mfsimvr_system_info::try_get_permission("android.permission.RECORD_AUDIO");
                    mfsimvr_system_info::try_get_permission("com.picovr.permission.FACE_TRACKING")
                }
            }
        }
        ...
        {
            #[cfg(target_os = "android")]
            mfsimvr_system_info::try_get_permission("com.oculus.permission.BODY_TRACKING")
        }
