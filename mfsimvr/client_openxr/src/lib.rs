...
use mfsimvr_client_core::{ClientCapabilities, ClientCoreContext, ClientCoreEvent};
use mfsimvr_common::{
...
use mfsimvr_graphics::GraphicsContext;
use mfsimvr_session::{BodyTrackingBDConfig, BodyTrackingSourcesConfig, PerformanceLevel};
use mfsimvr_system_info::Platform;
...
pub fn entry_point() {
    ...
    let (loader_suffix, openxr_version) = match mfsimvr_system_info::platform(None, None) {
    ...
    let platform = mfsimvr_system_info::platform(
        xr_instance
            .properties()
            .ok()
