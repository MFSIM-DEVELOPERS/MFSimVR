...
use mfsimvr_common::{ToAny, anyhow::Result, semver::Version};
...
fn install_and_launch_apk(
    ...
    let apk_name = "mfsimvr_client_android.apk";
    ...
    if !apk_path.exists() {
        ...
        let apk_buffer = mfsimvr_adb::commands::download(apk_url, |downloaded, total| {
    ...
    let layout = mfsimvr_filesystem::Layout::new(&root);
    let adb_path = mfsimvr_adb::commands::require_adb(&layout, |downloaded, total| {
        let progress = total.map_or(0.0, |t| downloaded as f32 / t as f32);
        worker_message_sender
            .send(WorkerMessage::ProgressUpdate(Progress {
    ...
    let device_serial = mfsimvr_adb::commands::list_devices(&adb_path)?
        .iter()
    ...
    let application_id = if stable {
        mfsimvr_system_info::PACKAGE_NAME_GITHUB_STABLE
    } else {
        mfsimvr_system_info::PACKAGE_NAME_GITHUB_DEV
    };

    if mfsimvr_adb::commands::is_package_installed(&adb_path, &device_serial, application_id)? {
        worker_message_sender.send(WorkerMessage::ProgressUpdate(Progress {
            message: "Uninstalling old APK".into(),
            progress: 0.0,
        }))?;
        mfsimvr_adb::commands::uninstall_package(&adb_path, &device_serial, application_id)?;
    }
    ...
    mfsimvr_adb::commands::install_package(&adb_path, &device_serial, &apk_path.to_string_lossy())?;

    mfsimvr_adb::commands::start_application(&adb_path, &device_serial, application_id)?;
    ...
    if let Some(session_version) = session_version {
        ...
        for inst in get_installations() {
            if inst.version == session_version {
                let source = mfsimvr_filesystem::filesystem_layout_from_openvr_driver_root_dir(
                    &installations_dir().join(session_version),
                )
                .unwrap()
                .session();

                let destination = mfsimvr_filesystem::filesystem_layout_from_openvr_driver_root_dir(
                    &installation_dir,
                )
                .unwrap()
                .session();
...
pub fn data_dir() -> PathBuf {
    if cfg!(target_os = "linux") {
        PathBuf::from(env::var("HOME").expect("Failed to determine home directory"))
            .join(".local/share/MFSimVR-Launcher")
    } else {
        env::current_exe()
                ...
                entry
                    ...
                    .map(|entry| {
                        let has_session_json = if cfg!(windows) {
                            mfsimvr_filesystem::filesystem_layout_from_openvr_driver_root_dir(
                                &entry.path(),
                            )
...
pub fn launch_dashboard(version: &str) -> Result<()> {
    ...
    let dashboard_path = if cfg!(windows) {
        installation_dir.join("mfsimvr Dashboard.exe")
    } else if cfg!(target_os = "linux") {
        installation_dir.join("mfsimvr_streamer_linux/bin/mfsimvr_dashboard")
    } else {
