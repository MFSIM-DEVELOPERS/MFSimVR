...
use mfsimvr_events::{EventType, HapticsEvent};
use mfsimvr_filesystem as afs;
use mfsimvr_packets::{
...
use mfsimvr_session::{CodecType, H264Profile, OpenvrProperty, Settings, SteamvrHmdInitConfig};
use mfsimvr_sockets::StreamSender;
...
pub fn notify_restart_driver() {
    if sysinfo::System::new_all()
        ...
        .is_some()
    {
        mfsimvr_events::send_event(EventType::ServerRequestsSelfRestart);
    } else {
...
impl ServerCoreContext {
    pub fn new() -> (Self, mpsc::Receiver<ServerCoreEvent>) {
        ...
        webserver_runtime.spawn({
            ...
            async move { mfsimvr_common::show_err(web_server::web_server(connection_context).await) }
        });
    ...
    pub fn send_haptics(&self, haptics: Haptics) {
        ...
        let haptics_config = {
            ...
            if session_manager_lock.settings().extra.logging.log_haptics {
                mfsimvr_events::send_event(EventType::Haptics(HapticsEvent {
