...
use mfsimvr_packets::{ButtonEntry, ButtonValue, Haptics};
use mfsimvr_session::CodecType;
use std::{
...
pub unsafe extern "C" fn alvr_path_to_id(path_string: *const c_char) -> u64 {
    mfsimvr_common::hash_string(unsafe { CStr::from_ptr(path_string) }.to_str().unwrap())
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn alvr_error(string_ptr: *const c_char) {
    mfsimvr_common::show_e(unsafe { CStr::from_ptr(string_ptr) }.to_string_lossy());
}
...
pub unsafe extern "C" fn alvr_dbg_server_impl(string_ptr: *const c_char) {
    mfsimvr_common::dbg_server_impl!(
        "{}",
        unsafe { CStr::from_ptr(string_ptr) }.to_string_lossy()
    );
}
...
pub unsafe extern "C" fn alvr_initialize_environment(
    ...
    crate::initialize_environment(mfsimvr_filesystem::Layout {
        config_dir,
        log_dir,
        ..Default::default()
    });
        ...
        match event {
            ...
            ServerCoreEvent::LocalViewParams(config) => unsafe {
                *out_event = AlvrEvent::LocalViewParams([
                    mfsimvr_common::to_capi_view_params(&config[0]),
                    mfsimvr_common::to_capi_view_params(&config[1]),
                ])
            },
            ...
            context.get_device_motion(device_id, Duration::from_nanos(sample_timestamp_ns))
    {
        unsafe {
            *out_motion = AlvrDeviceMotion {
                pose: mfsimvr_common::to_capi_pose(&motion.pose),
...
pub unsafe extern "C" fn alvr_get_hand_skeleton(
        ...
        )
    {
        for (i, joint_pose) in skeleton.iter().enumerate() {
            unsafe { *out_skeleton.add(i) = mfsimvr_common::to_capi_pose(joint_pose) };
        }
...
pub unsafe extern "C" fn alvr_send_video_nal(
    ...
    if let Some(context) = &*SERVER_CORE_CONTEXT.read() {
        ...
        let global_view_params = unsafe {
            [
                mfsimvr_common::from_capi_view_params(&(*global_view_params)),
                mfsimvr_common::from_capi_view_params(&(*global_view_params.add(1))),
            ]
        };
