...
use mfsimvr_events::{EventType, TrackingEvent};
use mfsimvr_packets::TrackingData;
use mfsimvr_session::{
    BodyTrackingConfig, HeadsetConfig, RecenteringMode, Settings, VMCConfig,
    settings_schema::Switch,
};
use mfsimvr_sockets::StreamReceiver;
use std::{
    cmp::Ordering,
    ...
    while is_streaming() {
        ...
        let device_motion_keys;
        {
            ...
            if session_manager_lock.settings().extra.logging.log_tracking {
                ...
                mfsimvr_events::send_event(EventType::Tracking(Box::new(TrackingEvent {
                    device_motions,
