use mfsimvr_common::{HEAD_ID, SlidingWindowAverage};
use mfsimvr_events::{BitrateDirectives, EventType, GraphStatistics, StatisticsSummary};
use mfsimvr_packets::ClientStatistics;
use std::{
    ...
    pub fn report_statistics(&mut self, client_stats: ClientStatistics) -> (Duration, Duration) {
            ...
            .find(|frame| frame.target_timestamp == client_stats.target_timestamp)
        {
            ...
            if self.last_full_report_instant + FULL_REPORT_INTERVAL < Instant::now() {
                ...
                mfsimvr_events::send_event(EventType::StatisticsSummary(StatisticsSummary {
            ...
            mfsimvr_events::send_event(EventType::GraphStatistics(GraphStatistics {
