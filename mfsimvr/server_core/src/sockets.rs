...
impl WelcomeSocket {
    pub fn new() -> Result<Self> {
        let mdns_receiver = ServiceDaemon::new()?.browse(mfsimvr_sockets::MDNS_SERVICE_TYPE)?;
            ...
            match self.mdns_receiver.try_recv() {
                Ok(event) => {
                    if let ServiceEvent::ServiceResolved(info) = event {
                        let hostname = info
                            .get_property_val_str(mfsimvr_sockets::MDNS_DEVICE_ID_KEY)
                            .unwrap_or_else(|| info.get_hostname());
                        let address = info.get_addresses().iter().next().to_any()?;

                        let client_protocol = info
                            .get_property_val_str(mfsimvr_sockets::MDNS_PROTOCOL_KEY)
                            .to_any()?;
                        let server_protocol = mfsimvr_common::protocol_id();
