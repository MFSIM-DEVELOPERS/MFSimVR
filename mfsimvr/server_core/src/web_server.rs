...
use mfsimvr_common::{ConnectionState, LogEntry, anyhow::Result, error, info, log};
use mfsimvr_events::{ButtonEvent, EventType};
use mfsimvr_packets::{ButtonEntry, ClientConnectionsAction, FirewallRulesAction, PathValuePair};
use mfsimvr_session::SessionConfig;
use axum::{
    Json, Router,
    ...
    let router = Router::new()
        .nest(
            ...
            Router::new()
                ...
                .route(
                    "/version",
                    routing::get(async || mfsimvr_common::MFSIMVR_VERSION.to_string()),
                )
...
async fn get_session() {
    mfsimvr_events::send_event(EventType::Session(Box::new(
...
async fn add_firewall_rules() {
    if let Err(e) =
        mfsimvr_server_io::firewall_rules(FirewallRulesAction::Add, FILESYSTEM_LAYOUT.get().unwrap())
    {
...
async fn get_driver_list() {
    if let Ok(list) = mfsimvr_server_io::get_registered_drivers() {
        mfsimvr_events::send_event(EventType::DriversList(list));
    }
}
...
async fn set_buttons(
    ...
    let button_entries = button_events
        .iter()
        .map(|b| ButtonEntry {
            path_id: mfsimvr_common::hash_string(&b.path),
            value: b.value,
        })
        .collect();
