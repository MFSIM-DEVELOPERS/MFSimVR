...
use mfsimvr_common::{LogEntry, LogSeverity, log::LevelFilter};
...
pub fn init_logging(session_log_path: Option<PathBuf>, crash_log_path: Option<PathBuf>) {
    ...
    let mut log_dispatch = Dispatch::new()
            ...
            move |meta| {
                !meta.target().starts_with("mdns_sd")
                    && (meta.level() <= LevelFilter::Info
                        || mfsimvr_common::filter_debug_groups(meta.target(), &debug_groups_config))
            }
        })
        .format(move |out, message, record| {
            ...
            let event_type = if maybe_event.starts_with('{') && maybe_event.ends_with('}') {
                ...
                && mfsimvr_common::is_enabled_debug_group(record.target(), &debug_groups_config)
            {
    ...
    mfsimvr_common::set_popup_callback(popup_callback);

    mfsimvr_common::set_panic_hook();
}
