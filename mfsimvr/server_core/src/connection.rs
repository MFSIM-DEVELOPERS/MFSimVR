...
use mfsimvr_adb::{WiredConnection, WiredConnectionStatus};
use mfsimvr_common::{
...
use mfsimvr_events::{AdbEvent, ButtonEvent, EventType};
use mfsimvr_packets::{
...
pub fn handshake_loop(ctx: Arc<ConnectionContext>, lifecycle_state: Arc<RwLock<LifecycleState>>) {
                ...
                let connection = match WiredConnection::new(
                    ...
                    |downloaded, maybe_total| {
                        if let Some(total) = maybe_total {
                            mfsimvr_events::send_event(EventType::Adb(AdbEvent {
    ...
    mfsimvr_common::dbg_connection!("handshake_loop: Joining connection threads");
    ...
    mfsimvr_common::dbg_connection!("handshake_loop: End");
}
...
fn try_connect(
    ...
    let (socket, client_ip, connection_result) = mfsimvr_sockets::connect_to_client(
...
fn connection_pipeline(
    ...
    let maybe_streaming_caps =
        if let ClientConnectionResult::ConnectionAccepted(info) = connection_result {
            ...
            if info.client_protocol_id != mfsimvr_common::protocol_id_u64() {
                warn!(
                    "Trusted client is incompatible! Expected protocol ID: {}, found: {}",
                    mfsimvr_common::protocol_id_u64(),
    ...
    let game_audio_sample_rate =
        if let Switch::Enabled(game_audio_config) = &initial_settings.audio.game_audio {
            let game_audio_device =
                mfsimvr_audio::new_output(game_audio_config.device.as_ref()).to_con()?;

            if let Switch::Enabled(microphone_config) = &initial_settings.audio.microphone
                && matches!(
                    microphone_config.devices,
                    mfsimvr_session::MicrophoneDevicesConfig::VAC
                        | mfsimvr_session::MicrophoneDevicesConfig::VBCable
                )
            {
                let (sink, _) =
                    mfsimvr_audio::new_virtual_microphone_pair(microphone_config.devices.clone())
                        .to_con()?;
                ...
                if mfsimvr_audio::is_same_device(&game_audio_device, &sink) {
                    con_bail!("Game audio and microphone cannot point to the same device!");
                }
            }

            mfsimvr_audio::input_sample_rate(&game_audio_device).to_con()?
        } else {
            0
        };
    ...
    if session_manager_lock.session().restart_settings_hash != new_hash {
        ...
        mfsimvr_sockets::send_restart_signal(socket, stream_config_packet)?;
    ...
    *ctx.tracking_manager.write() =
    ...
    let game_audio_sender: mfsimvr_sockets::StreamSender<()> = socket.request_unreliable_stream(AUDIO);
    let haptics_sender = socket.request_unreliable_stream(HAPTICS);

    let mut control_receiver = socket.subscribe_to_reliable_stream()?;
    let mut microphone_receiver: mfsimvr_sockets::StreamReceiver<()> =
    ...
    *ctx.haptics_sender.lock() = Some(haptics_sender);
        ...
        initial_settings.audio.game_audio.clone()
    {
        ...
        thread::spawn(move || {
            ...
            while is_streaming(&client_hostname) {
                {
                    let device = match mfsimvr_audio::new_output(config.device.as_ref()) {
                        Ok(data) => data,
                        Err(e) => {
                    ...
                    #[cfg(windows)]
                    if let Ok(id) = mfsimvr_audio::get_windows_device_id(&device) {
                        let prop = mfsimvr_session::OpenvrProperty {
                            key: mfsimvr_session::OpenvrPropKey::AudioDefaultPlaybackDeviceIdString,
                            value: id,
                        };
                        ctx.events_sender
                            .send(ServerCoreEvent::SetOpenvrProperty {
                                device_id: *mfsimvr_common::HEAD_ID,
                                prop,
                            })
                            .ok();
                    } else {
                    ...
                    if let Err(e) = mfsimvr_audio::record_audio_blocking(
                        Arc::new({
                            let client_hostname = client_hostname.clone();
                    ...
                    #[cfg(windows)]
                    if let Ok(id) = mfsimvr_audio::new_output(None)
                        .and_then(|d| mfsimvr_audio::get_windows_device_id(&d))
                    {
                        let prop = mfsimvr_session::OpenvrProperty {
                            key: mfsimvr_session::OpenvrPropKey::AudioDefaultPlaybackDeviceIdString,
                            value: id,
        ...
        initial_settings.audio.microphone.clone()
    {
        ...
        let (sink, source) = mfsimvr_audio::new_virtual_microphone_pair(config.devices).to_con()?;

        #[cfg(windows)]
        if let Ok(id) = mfsimvr_audio::get_windows_device_id(&source) {
            ctx.events_sender
                .send(ServerCoreEvent::SetOpenvrProperty {
                    device_id: *mfsimvr_common::HEAD_ID,
                    prop: mfsimvr_session::OpenvrProperty {
                        key: mfsimvr_session::OpenvrPropKey::AudioDefaultRecordingDeviceIdString,
                        value: id,
                    },
                })
                .ok();
        ...
        let client_hostname = client_hostname.clone();
        thread::spawn(move || {
            mfsimvr_common::show_err(mfsimvr_audio::play_audio_loop(
                {
                    let client_hostname = client_hostname.clone();
                    move || is_streaming(&client_hostname)
                },
    ...
    let microphone_thread = {
        use mfsimvr_audio::linux::{self, AudioInfo};
            ...
            if let Some(mappings) = &config.button_mappings {
                ...
                ButtonMappingManager::new_automatic(
                    &CONTROLLER_PROFILE_INFO
                        .get(&mfsimvr_common::hash_string(QUEST_CONTROLLER_PROFILE_PATH))
                        .unwrap()
                        .button_set,
                    ...
                    ClientControlPacket::Buttons(entries) => {
                        {
                            ...
                            if session_manager_lock
                                ...
                                .log_button_presses
                            {
                                mfsimvr_events::send_event(EventType::Buttons(
                                    entries
                                        .iter()
                                        .map(|e| ButtonEvent {
    ...
    mfsimvr_common::wait_rwlock(&disconnect_notif, &mut session_manager_lock);
