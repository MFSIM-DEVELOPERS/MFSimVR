...
use mfsimvr_events::EventType;
use mfsimvr_packets::{ClientConnectionsAction, PathSegment, PathValuePair};
use mfsimvr_session::{ClientConnectionConfig, SessionConfig, Settings};
...
impl Drop for SessionLock<'_> {
    fn drop(&mut self) {
        ...
        *self.settings = self.session_desc.to_settings();
        mfsimvr_events::send_event(EventType::Session(Box::new(self.session_desc.clone())));
    }
}
...
impl ServerSessionManager {
    ...
    pub fn set_session_values(&mut self, descs: Vec<PathValuePair>) -> Result<()> {
        ...
        mfsimvr_events::send_event(EventType::Session(Box::new(self.session_config.clone())));
